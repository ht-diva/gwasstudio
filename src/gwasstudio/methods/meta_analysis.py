...
def _meta_analysis(tiledb_array, trait_list, out_prefix=None, **kwargs):
    ...
    effect_sizes = np.column_stack(
        [merged_df["BETA"].values] + [merged_df[f"BETA_{i + 1}"].values for i in range(1, len(trait_list) - 1)]
    )

    standard_error = np.column_stack(
        [merged_df["SE"].values] + [merged_df[f"SE_{i}"].values for i in range(1, len(trait_list) - 1)]
    )
